...
export default function QuoteTableHead({ dispatch, sortParams }: QuoteTableHeadProps) {
  return (
    <thead>
      <tr>
        { quoteTableFields.map(tableField => {
          ...
          return (
            <th className={sortClass} key={ tableField.field } onClick={ () => {
              dispatch(tickersSlice.actions.setSortType({
                type: sortParams.field === tableField.field && sortParams.type === 'down' ? 'up' : 'down',
